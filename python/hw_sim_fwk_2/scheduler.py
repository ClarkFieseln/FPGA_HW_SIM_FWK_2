...
class Scheduler:
    ...
    def do_slot(self, slot_nr, input_fifo):
        ...
        if led_start != -1:
            self.leds.do_slot(slot_nr, input_fifo[led_start +
                                                  len(self.LED_HEADER):led_start +
                                                  len(self.LED_HEADER) +
                                                  configuration.NR_LEDS])
        do_start = input_fifo.find(self.DO_HEADER)
        if do_start != -1:
            self.digital_outputs.do_slot(slot_nr, input_fifo[do_start +
                                                             len(self.DO_HEADER):do_start +
                                                             len(self.DO_HEADER) +
    ...
    def thread_scheduler(self):
        ...
        self.fifo_app_to_sim = create_w_fifo(self.FILE_NAME_FIFO_APP_TO_SIM, self.__event.evt_close_app)
        # create FIFO to RX data from simulator (blocking call)
        self.fifo_sim_to_app = create_r_fifo(self.FILE_NAME_FIFO_SIM_TO_APP, self.__event.evt_close_app)
        ...
        while (self.__event.evt_power_on.is_set() is False) and (self.__event.evt_close_app.is_set() is False):
            time.sleep(configuration.POLL_DELAY_SEC)
        if self.__event.evt_close_app.is_set() is False:
            ...
            win32file.ReadFile(self.fifo_sim_to_app, configuration.FIFO_READ_BUFFER_SIZE)
            ...
            while self.__event.evt_close_app.is_set() is False:
                ...
                if self.__event.evt_pause.is_set() is False:
                    ...
                    if self.time_slot == SLOT_CLOCK_RAISING_EDGE:                        
                        ...
                        line = win32file.ReadFile(self.fifo_sim_to_app, configuration.FIFO_READ_BUFFER_SIZE)
                    ...
                    ##################
                        ...
                        if report is True:
                            win32file.WriteFile(self.fifo_app_to_sim, str.encode(reset_signal +
                                                                                 buttons_signal + switches_signal +
                                                                                 digital_inputs_signal + "\r\n"))
                        else:
